...
FORBIDDEN_FOODS = ["chocolate", "candy", "expired milk", "expired meat", "rotten fish", "cake"]
PICKY_CHANCE = 0.35
...
def handle_sleeping_state(user_input: str) -> bool:
    ...
    if not cat.is_sleeping:
        return False
    if has_keywords(["hit","slap"],user_input):
        cat.is_sleeping= False
        cat.trust = max(0, cat.trust - 2)
        set_emotion("angry", 5)
        cat.closeness = 3
        print("hits your face")
        print("jumps away")
        return False            
    
    if has_keywords(["wake","get up"], user_input):
        cat.is_sleeping = False
...
def handle_very_sick_state(user_input: str) -> bool:
    ...
    if has_keywords(["pet", "hug", "cuddle", "are you okay", "status", "check","gives her a hugg"], user_input):
...
def main():
    ...
    while True:
        ...
        if "i am" in user_input:
            ...
            if not cat.recognized and user_input in ["i am nikhil"]:
                cat.recognized = True
                print("poppu smells you and remembers you as its creator ")
